...
const flush_stats = function coralogix_post_stats(ts, metrics) {
    ...
    for (full_key in counters) {
        ...
        const valuePerSecond = value / (flushInterval / 1000); // calculate 'per second' rate

        payload.push({
            labels: make_labels(mapping?.labels, host, get_prefix(key) + '_total').concat(labels_from_tags),
            samples: [{ timestamp: timestamp, value: valuePerSecond }],
        });
    }
    ...
    for (full_key in timers) {
        ...
        if (timers[full_key].length > 0) {
            mapping = coralogixMappings.get(key);
            const values = timers[full_key].sort(function (a, b) { return a - b; });
            ...
            if (buckets) {
                ...
                for (bucket in buckets) {
                    const bucket_count = values.filter(function (v) { return v <= buckets[bucket]; }).length;
